...
export default function CodeProject() {
  ...
  const xNValues: number[] = [];
  const yNValues: number[] = [];
  const xPValues: number[] = [];
  const yPValues: number[] = [];

  // Calculate positions for 1/4 circle (90°)
  for (let i = 0; i <= steps; i++) {
    const angle = (i / steps) * (Math.PI / 2); // Only 90 degrees (PI/2)
    xNValues.push(-radius * Math.cos(angle)); // Start from left (-radius)
    yNValues.push(-radius * Math.sin(angle)); // Move upwards
    xPValues.push(radius * Math.cos(angle)); // Start from left (-radius)
    yPValues.push(-radius * Math.sin(angle));
  }
  ...
  const getAnimatedValues = () => {
    if (animateNext) {
      return { x: xNValues, y: yNValues };
    } else {
      return { x: xPValues, y: yPValues };
    }
  };
  ...
  return (
    <>
      ...
      <section className="max-w-screen-2xl mx-auto z-30 w-full flex flex-col items-center justify-center h-screen relative ">
